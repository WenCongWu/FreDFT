...
class YOLOV5TorchObjectDetector(nn.Module):
    def __init__(self,
        ...
        img1 = torch.zeros((1, 3, *self.img_size), device=device)
        #self.model(img)
        self.model(img, img1)
    ...
    def forward(self, img, img_ir):
        ...
        prediction, logits, _ = self.model(img, img_ir, augment=False)
    ...
    def preprocessing(self, img, img_ir):
        if len(img.shape) != 4:
        ...
        if len(img_ir.shape) != 4:
            img_ir = np.expand_dims(img_ir, axis=0)
        im0_ir = img_ir.astype(np.uint8)
        #img_ir = np.array([self.yolo_resize(im, new_shape=self.img_size)[0] for im in im0_ir])
        img_ir = im0_ir
        img_ir = img_ir.transpose((0, 3, 1, 2))
        img_ir = np.ascontiguousarray(img_ir)
        img_ir = torch.from_numpy(img_ir).to(self.device)
        img_ir = img_ir / 255.0
